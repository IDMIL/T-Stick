//********************************************************************************//
//  Sopranino T-Stick 2GW - LOLIN D32 PRO - USB -WiFi // Input Devices and Music
//  Interaction Laboratory (IDMIL)                        // Created:  February
//  2018 by Alex Nieva                                         //
//            October 2019 by Edu Meneses - firmware version 200214
//            (2020/Feb/20) //
...
// IMPORTANT: You need to upload a file (data/config.json) into ESP32
// filesystem. Follow the instructions at
...
//  OBS:
//  1-) Use esp32 1.0.4 or newer
//  (https://github.com/espressif/arduino-esp32/releases). 2-) Also install
//  ESP8266 board library even if you'll use the ESP32
//  (https://github.com/esp8266/Arduino) 3-) MINU library complains if you keep
//  any IMU-related files other than MIMU_LSM9DS1.h and MIMU_LSM9DS1.cpp 4-)
//  Board currently in use: LOLIN D32 PRO.
...
WiFiUDP
    oscEndpoint;  // A UDP instance to let us send and receive packets over UDP
const unsigned int portLocal =
    8888;  // local port to listen for OSC packets (not used for sending)
...
int waitForConnection =
    8000;                   // set waiting time for connecting to a WiFi network
...
unsigned long time_now =
    millis();  // variable used for LED, serial print, battery reading, and WiFi
               // connection (updated by blinkLED() every loop)
float fsrbuf;
...
void setup() {
  ...
  Serial.println(
      ...
      "        *");
  Serial.println(
      "*           February 2020 by Edu Meneses - firmware version 200207 "
      "(2020-02-20) *");
  Serial.println(
      "*  Notes:   Based on test program for reading CY8C201xx using I2C       "
      "        *");
