...
void libmapperPollTask(void* user_param) {
  for (;;) {
    mpr_dev_poll(user_param, 0);
    portYIELD();
  }
}
...
void initLibmapper() {
  ...
  xTaskCreate(libmapperPollTask, "libmpr", 16384, libmapper_dev,
              3 | portPRIVILEGE_BIT, NULL);
  ...
  float normMin = -1.0f, normMax = 1.0f;
  ...
  sigRawGyroX = mpr_sig_new(libmapper_dev, MPR_DIR_OUT, "raw/gyro/X", 1,
                            MPR_FLT, 0, &normMin, &normMax, 0, 0, 0);
...
void updateLibmapper() {
  ...
  // mpr_sig_set_value(sigRawCapsense, 0, sizeof(touchtemp), MPR_INT32,
  // touchtemp,
  //                   MPR_NOW);
  ...
  mpr_sig_set_value(sigRawAcclX, 0, 1, MPR_FLT, &NormData.accl[0], MPR_NOW);
  mpr_sig_set_value(sigRawAcclY, 0, 1, MPR_FLT, &NormData.accl[1], MPR_NOW);
  mpr_sig_set_value(sigRawAcclZ, 0, 1, MPR_FLT, &NormData.accl[2], MPR_NOW);
  mpr_sig_set_value(sigRawGyroX, 0, 1, MPR_FLT, &NormData.gyro[0], MPR_NOW);
  mpr_sig_set_value(sigRawGyroY, 0, 1, MPR_FLT, &NormData.gyro[1], MPR_NOW);
  mpr_sig_set_value(sigRawGyroZ, 0, 1, MPR_FLT, &NormData.gyro[2], MPR_NOW);
  mpr_sig_set_value(sigRawMagnX, 0, 1, MPR_FLT, &NormData.magn[0], MPR_NOW);
  mpr_sig_set_value(sigRawMagnY, 0, 1, MPR_FLT, &NormData.magn[1], MPR_NOW);
  mpr_sig_set_value(sigRawMagnZ, 0, 1, MPR_FLT, &NormData.magn[2], MPR_NOW);
  // mpr_sig_set_value(sigQuarternion1, 0, 1, MPR_FLT, &RawData.quat[0],
  // MPR_NOW); mpr_sig_set_value(sigQuarternion2, 0, 1, MPR_FLT,
  // &RawData.quat[1], MPR_NOW); mpr_sig_set_value(sigQuarternion3, 0, 1,
  // MPR_FLT, &RawData.quat[2], MPR_NOW); mpr_sig_set_value(sigQuarternion4, 0,
  // 1, MPR_FLT, &RawData.quat[3], MPR_NOW); mpr_sig_set_value(sigMagGyro, 0, 1,
  // MPR_FLT, &RawData.magGyro, MPR_NOW); mpr_sig_set_value(sigMagAccl, 0, 1,
  // MPR_FLT, &RawData.magAccl, MPR_NOW); mpr_sig_set_value(sigMagMagn, 0, 1,
  // MPR_FLT, &RawData.magMagn, MPR_NOW);
  mpr_sig_set_value(sigButton, 0, 1, MPR_INT32, &RawData.buttonState, MPR_NOW);
