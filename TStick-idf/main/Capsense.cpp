...
Capsense capsenseRequest(uint8_t address, uint8_t request,
                         uint8_t answer_size) {
...
void initCapsense(byte I2C_ADDR) {
  ...
  Serial.print("\nChecking capsense config (0x");
  Serial.print(I2C_ADDR, HEX);
  Serial.println("):");
  // Asking for Capsense System Status
  Serial.print("- Send a request: Begin Transmission to address 0x");
  Serial.print(I2C_ADDR, HEX);
  Serial.print(" (");
  ...
  if (capsense.answer1 == 0) {
    Serial.println(
        "  A configuration other than the factory default configuration is "
        "loaded");
  } else if (capsense.answer1 == 1) {
    Serial.println("  The factory default configuration is loaded");
  } else {
    Serial.print("  Unknown error (weird capsense response): SYSTEM_STATUS: ");
  ...
  Serial.print(capsense.answer1);
  Serial.print(" ");
  Serial.println(capsense.answer2);
}
...
void capsense_scan() {
  ...
  if (nCapsenses == 0) {
    Serial.println(
        "\n\nOops ... unable to initialize IDMIL's Capsense. Check your "
        "wiring/board!\n\n");
  } else {
