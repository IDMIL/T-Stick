...
void connectToWifi() {
  WiFi.mode(WIFI_STA);  // Force to station mode because if device was switched
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      Serial.println(
          "Failed to connect to the saved network. Enter setup to connect to a "
          "different network.");
    }
  } else {
    Serial.print("The T-Stick is connected to ");
    Serial.print(WiFi.SSID());
    Serial.println(" network");
    Serial.print("IP address: ");
    Serial.println(WiFi.localIP());
...
void Wifimanager_portal(char *portal_name, char *portal_password) {
  ...
  WiFiManagerParameter wifimanager_APpasswd("APpasswd", "T-Stick password",
                                            APpasswdTemp, 15);
  WiFiManagerParameter wifimanager_APpasswdValidate(
      "APpasswdValidate", "Type T-Stick password again", APpasswdValidate, 15);
  WiFiManagerParameter wifimanager_hint(
      "<small>*Hint: if you want to reuse the currently active WiFi "
      "credentials, leave Password fields empty</small>");

  snprintf(wifimanagerbuf, (sizeof(wifimanagerbuf) - 1), "%s,%s",
           Tstick.oscIP[0], Tstick.oscIP[1]);
  WiFiManagerParameter wifimanager_oscIP("server", "IP to send OSC messages",
                                         wifimanagerbuf, 40);

  snprintf(wifimanagerbuf, (sizeof(wifimanagerbuf) - 1), "%i,%i",
           Tstick.oscPORT[0], Tstick.oscPORT[1]);
  WiFiManagerParameter wifimanager_oscPORT("port", "port to send OSC messages",
                                           wifimanagerbuf, 16);

  if (Tstick.libmapper == 1) {
    strcat(libmapperCheck, " checked");
  }
  WiFiManagerParameter wifimanager_libmapper(
      "libmapper", "Libmapper ON/OFF (WiFiManager exit required)", "T", 2,
      libmapperCheck, WFM_LABEL_AFTER);

  if (Tstick.osc == 1) {
    strcat(OSCCheck, " checked");
  }
  WiFiManagerParameter wifimanager_osc(
      "OSC", "OSC messages ON/OFF (WiFiManager exit required)", "T", 2,
      OSCCheck, WFM_LABEL_AFTER);

  fsrbuf = float(Tstick.FSRoffset) / 4095;
  WiFiManagerParameter wifimanager_FSRoffset(
      "FSRoffset", "FSR offset value (default = 0)",
      dtostrf(fsrbuf, 2, 3, wifimanagerbuf), 6);

  snprintf(
      wifimanagerbuf, (sizeof(wifimanagerbuf) - 1),
      "%i,%i,%i,%i,%i,%i,%i,%i,%i,%i", Tstick.touchMask[0][0],
      ...
      Tstick.touchMask[3][1], Tstick.touchMask[4][0], Tstick.touchMask[4][1]);

  WiFiManagerParameter wifimanager_touchMask(
      "touchMask", "Touch Mask capacitive sensing values", wifimanagerbuf, 40);

  WiFiManagerParameter wifimanager_id("id", "T-Stick serial number",
                                      itoa(Tstick.id, wifimanagerbuf, 10), 6,
                                      "readonly");
  WiFiManagerParameter wifimanager_type("type", "T-Stick size (type)",
                                        Tstick.type, 4, "readonly");
  WiFiManagerParameter wifimanager_author("author", "T-Stick firmware revision",
                                          Tstick.author, 20, "readonly");
  WiFiManagerParameter wifimanager_color("color", "T-Stick color", Tstick.color,
                                         10, "readonly");
  WiFiManagerParameter wifimanager_firmware(
      "firmware", "T-Stick firmware revision",
      itoa(Tstick.firmware, wifimanagerbuf, 10), 10, "readonly");
  ...
  if (wifimanager_APpasswd.getValue()[0] != NULL) {
    if (strcmp(wifimanager_APpasswd.getValue(),
               wifimanager_APpasswdValidate.getValue()) == 0) {
      strcpy(Tstick.APpasswd, wifimanager_APpasswd.getValue());
      Serial.println("T-Stick AP password changed");
    } else {
      Serial.println(
          "T-Stick AP passwords don't match! Not saving new password.");
    }
  } else {
    Serial.println(
        "T-Stick AP password fields are blank. Not saving new password.");
  }
  WiFi.SSID().toCharArray(Tstick.lastConnectedNetwork,
                          sizeof(Tstick.lastConnectedNetwork));
  ...
  if (copybuf != NULL) {
    ...
    for (byte i = 1;
         i < (sizeof(Tstick.touchMask) / sizeof(Tstick.touchMask)[0]); ++i) {
      copybuf = strtok(NULL, ",");
      if (copybuf == NULL) {
        break;
      }
