...
void printJSON() {
  if (SPIFFS.exists("/config.json")) {  // file exists, reading and loading
    ...
    File configFile =
        SPIFFS.open("/config.json", "r");  // Open file for reading
    if (configFile) {
      ...
      Serial.println(
          "Cannot read config file config.json. File doesn't exist.\n");
    }
  }
}
...
void printVariables() {
  ...
  Serial.print("Tstick.id: ");
  Serial.println(Tstick.id);
  Serial.print("Tstick.type: ");
  Serial.println(Tstick.type);
  Serial.print("Tstick.author: ");
  Serial.println(Tstick.author);
  Serial.print("Tstick.color: ");
  Serial.println(Tstick.color);
  Serial.print("Tstick.APpasswd: ");
  Serial.println(Tstick.APpasswd);
  Serial.print("Tstick.lastConnectedNetwork: ");
  Serial.println(Tstick.lastConnectedNetwork);
  Serial.print("Tstick.lastStoredPsk: ");
  Serial.println(Tstick.lastStoredPsk);
  Serial.print("Tstick.firmware: ");
  Serial.println(Tstick.firmware);
  Serial.print("Tstick.osc: ");
  Serial.println(Tstick.osc);
  Serial.print("Tstick.oscIP #1: ");
  Serial.println(Tstick.oscIP[0]);
  Serial.print("Tstick.oscIP #2: ");
  Serial.println(Tstick.oscIP[1]);
  Serial.print("Tstick.oscPORT #1: ");
  Serial.println(Tstick.oscPORT[0]);
  Serial.print("Tstick.oscPORT #2: ");
  Serial.println(Tstick.oscPORT[1]);
  Serial.print("Tstick.libmapper: ");
  Serial.println(Tstick.libmapper);

  Serial.print("Tstick.FSRoffset: ");
  Serial.print(Tstick.FSRoffset);
  Serial.print(" (normalized: ");
  Serial.print(float(Tstick.FSRoffset) / 4095, 4);
  ;
  Serial.println(")");
  Serial.print("\nTstick.touchMask: ");
  for (int i = 0; i < (sizeof(Tstick.touchMask) / sizeof(Tstick.touchMask[0]));
       ++i) {
  ...
  for (int i = 0;
       i < (sizeof(Tstick.acclcalibration) / sizeof(Tstick.acclcalibration[0]));
       ++i) {
  ...
  for (int i = 0;
       i < (sizeof(Tstick.magncalibration) / sizeof(Tstick.magncalibration[0]));
       ++i) {
  ...
  for (int i = 0;
       i < (sizeof(Tstick.gyrocalibration) / sizeof(Tstick.gyrocalibration[0]));
       ++i) {
...
void parseJSON() {
  ...
  const size_t capacity = 5 * JSON_ARRAY_SIZE(2) + 3 * JSON_ARRAY_SIZE(3) +
                          3 * JSON_ARRAY_SIZE(9) + JSON_OBJECT_SIZE(26) + 360;
  ...
  if (SPIFFS.exists("/config.json")) {  // file exists, reading and loading
    ...
    if (error) {
      ...
      strlcpy(Tstick.lastConnectedNetwork, doc["lastConnectedNetwork"],
              sizeof(Tstick.lastConnectedNetwork));
      strlcpy(Tstick.lastStoredPsk, doc["lastStoredPsk"],
              sizeof(Tstick.lastStoredPsk));
