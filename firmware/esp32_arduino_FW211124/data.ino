...
//        }
    if (trillamount == 2) {
      trillSensor2.requestRawData();
      for (int i=0; i<touchStripsSize; i++) {
         if (trillSensor2.rawDataAvailable() > 0) {
              RawData.touch_trill2[i] = trillSensor2.rawDataRead();
         }
      }
    }
    ...
    if (trillamount == 2) {
      for (int i=touchStripsSize; i < touchStripsSize*trillamount; i++) {
            if (RawData.touch_trill2[i] != 0) {
                RawData.touchStrips[i] = 1;
            } else {
                RawData.touchStrips[i] = 0;
            }
