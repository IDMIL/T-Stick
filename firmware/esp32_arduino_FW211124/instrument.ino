...
void updateInstrument() {
  ...
  InstrumentData.touchAll = touchAverage(RawData.touchStrips, 0, touchStripsSize*trillamount);
  ...
  // normalized between 0 and 1
  InstrumentData.touchMiddle = touchAverage(RawData.touchStrips, (0+touchSizeEdge), ((touchStripsSize*trillamount) - touchSizeEdge));

  // InstrumentData.touchBottom: get the "amount of touch" for the botton part of the capsense
  // normalized between 0 and 1
  InstrumentData.touchBottom = touchAverage(RawData.touchStrips, ((touchStripsSize*trillamount)-touchSizeEdge), touchStripsSize*trillamount);
  ...
  #else
    BlobDetection = blobDetection1DTrill(RawData.touchStrips,touchStripsSize*trillamount);
  #endif
